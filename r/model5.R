...
games <- games[games$tournament_idx <= 1,]
...
mod_prior <- stan(file=file.path("stan_programs/model5_prior.stan"), 
...
mod <- stan(file=file.path("stan_programs/model5.stan"), 
              data=mod_data, warmup=500, iter=1000,seed = 9087872,
...
# player-wise skill averages
# gamma <- extract_mod$gamma
# skill_means <- data.frame(mean_skill=colMeans(gamma), player_idx=player_first_games$player_idx)
# games <- merge(games, skill_means, by.x='player_white_idx', by.y='player_idx')
# names(games)[names(games) == 'mean_skill'] <- 'mean_skill_player_white'
# games <- merge(games, skill_means, by.x='player_black_idx', by.y='player_idx')
# names(games)[names(games) == 'mean_skill'] <- 'mean_skill_player_black'
# 
# util$plot_conditional_mean_quantiles(mod_samples, pred_names,obs_xs =games$mean_skill_player_white - games$mean_skill_player_black, baseline_values = games$outcome_ordinal, display_ylim = c(0.5,3.5))
# util$plot_hist_quantiles(samples = mod_samples, bin_min = 0.5, bin_max = 3.5, bin_delta = 1,
#                          val_name_prefix = "y_pred", baseline_values = games$outcome_ordinal,
#                          xlab = "Outcome")
