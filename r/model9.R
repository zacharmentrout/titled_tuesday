...
clean_data <- util$clean_chess_data(games_init, max_tournament_idx = 10)
...
mod <- stan(file=file.path("stan_programs/model9.stan"), 
...
names <- c(
  paste0('gamma_free[', 1:(nrow(players)-1),',1]'),
  paste0('gamma_free[', 1:(nrow(players)-1),',2]'),
  paste0('cut_points[',1:2,']'),
  paste0('alpha[',1:(nrow(players)-1), ',1]'),
  paste0('alpha[',1:(nrow(players)-1), ',2]'),
  paste0('mu_alpha[',1:2,']'),
  'cor_wb',
...
# Population Mean
par(mfrow=c(1,1))
util$plot_expectand_pushforward(mod_samples[["mu_alpha[1]"]], 25,
...
par(mfrow=c(1,1))
util$plot_expectand_pushforward(mod_samples[["mu_alpha[2]"]], 25,
                                display_name="Population Mean", flim = c(-3,3))
xs <- seq(-3, 3, 0.01)
ys <- dnorm(xs,0, 1)
lines(xs, ys, lwd=2, col=c_light_teal)
...
# Correlation
par(mfrow=c(1,1))
util$plot_expectand_pushforward(mod_samples[["cor_wb"]], 25,
                                display_name="Correlations White-Black Skill", flim = c(-1,1))
xs <- seq(-1, 1, 0.01)
ys <- dunif(xs,0.5,1)
lines(xs, ys, lwd=2, col=c_light_teal)
...
for (c in c(2, 1241, 687, 633, 2197)) {
