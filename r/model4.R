...
mod <- stan(file=file.path("model4.stan"), 
...
names <- c(
  paste0('gamma[', 2:(nrow(player_first_games)),']'),
  'mu_gamma',
  'tau_gamma',
...
# player skill population location
par(mfrow=c(2,1), mar=c(5,5,1,1))
x <- seq(-10,10,by=0.1)
util$plot_expectand_pushforward(mod_samples[['mu_gamma']],
                                50, flim=c(min(x),max(x)),
                                display_name="mu_gamma")
lines(x, dnorm(x, 0, 3/2.32),col=util$c_mid_teal)

# population scale
x <- seq(0,3,by=0.1)
util$plot_expectand_pushforward(mod_samples[['tau_gamma']],
                                50, flim=c(min(x),max(x)),
                                display_name="tau_gamma")
lines(x, abs(dnorm(x, 0, 3/2.57)),col=util$c_mid_teal)
...
draw_percentages <- sapply(1:mod_data$N_players, function(x) { mean(games$outcome_ordinal[games$player_white_idx == x | games$player_black_idx == x] == 2) })
game_counts <- sapply(1:mod_data$N_players, function(x) { nrow(games[games$player_white_idx == x | games$player_black_idx == x,]) })
which(game_counts > 5 & draw_percentages > 0.3)

# A few players
par(mfrow=c(4, 2), mar=c(5, 5, 1, 1))
# for (c in sample(1:mod_data$N_players, 4)) {
for (c in c(51,  104,  113,  160)) {
...
n <- 1000
prior_means_gamma <- mean(rnorm(n, 0, 1) * abs(rnorm(n, 0, 3/2.57)) + rnorm(n, 0, 3/2.32))  #  delta_elo
...
contraction <- 1 - posterior_sds_gamma /  sd(rnorm(n, 0, 1) * abs(rnorm(n, 0, 3/2.57)) + rnorm(n, 0, 3/2.32))
...
plot(contraction, mean_gamma_shift, col=c("#8F272720"), lwd=2, pch=16, cex=0.8, main="Skill - Contraction vs. Mean Shift",
     xlim=c(0, 1), xlab="Posterior Contraction", ylim=c(-25, 5), ylab="Posterior Mean Shift")
