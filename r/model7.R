...
delta_elo <- (player_first_games$Elo - player1_elo)


games_prior_close_elo <- games_prior[abs(games_prior$WhiteElo - games_prior$BlackElo) < 200 & games_prior$Round == 1, ]
...
mean_outcome_player <-
  sapply(player_first_games$player_idx,
         function(c) mean(
           c(
             games$outcome_ordinal[games$player_white_idx == c],
             4-games$outcome_ordinal[games$player_black_idx == c])
            )
          )
util$plot_line_hist(mean_outcome_player,
                    0, 3.5, 0.25,
                    xlab="Player-wise Average Game Outcomes (White + Black)")
mean_outcome_player[is.nan(mean_outcome_player)] <- 0
...
draw_pct_player <-
  sapply(player_first_games$player_idx,
         function(c) mean(
           c(
             games$outcome_ordinal[games$player_white_idx == c] == 2,
             games$outcome_ordinal[games$player_black_idx == c] == 2)
         )
  )
util$plot_line_hist(draw_pct_player,
                    0, 1, 0.1,
                    xlab="Player Draw Percentage")
...
util$plot_line_hist(draw_count_player,
                    0, 40,
                    xlab="Player Draw Count")

which(draw_count_player > 10)
which(draw_pct_player > 0.3)
...
mod_prior <- stan(file=file.path("stan_programs/model6_prior.stan"), 
...
mod <- stan(file=file.path("stan_programs/model7.stan"), 
...
names <- c(
  ...
  paste0('alpha[',1:(nrow(player_first_games)-1), ']'),
  'mu_alpha',
  'tau_alpha',
  paste0('phi_free[', 1:(nrow(player_first_games)-1),']')
)
...
# Disc. Population Mean
par(mfrow=c(1,1))
util$plot_expectand_pushforward(mod_samples[["mu_phi"]], 25,
                                display_name="Population Mean - Discrimination", flim = c(-0.3,0.2))
xs <- seq(-0.3, 0.2, 0.01)
ys <- dnorm(xs,0, 0.15 / 2.32)
lines(xs, ys, lwd=2, col=c_light_teal)

# Disc. Population Scale
par(mfrow=c(1,1))
util$plot_expectand_pushforward(mod_samples[["tau_phi"]], 25,
                                display_name="Population Scale - Discrimination", flim = c(0,0.6))
xs <- seq(0, 0.6, 0.01)
ys <- abs(dnorm(xs,0, 0.15 / 2.57))
lines(xs, ys, lwd=2, col=c_light_teal)

util$my_q(mod_samples[["tau_phi"]])
util$my_q(abs(rnorm(1000,0,0.15/2.57)))
...
par(mfrow=c(1, 1), mar=c(5, 5, 1, 1))
null_player <- which(mean_outcome_player== 0)
pred_names_player <- paste0('mean_outcome_player_pred[',setdiff(1:nrow(player_first_games), null_black),']')
util$plot_hist_quantiles(mod_samples[pred_names_player], 'mean_outcome_player_pred',
                         0.5, 3.5, 0.5,
                         baseline_values=mean_outcome_player,
                         xlab="Player-wise Average Outcomes (White + Black)")
util$my_q(mean_outcome_player)
...
par(mfrow=c(1,1))
n <- 10000
mu_phi <- rnorm(1000, 0, 0.15 / 2.32)
tau_phi <- abs(rnorm(1000, 0, 0.3 / 2.57))
phi <- exp(rnorm(n, mean=mu_phi,sd=tau_phi))
hist(phi)
