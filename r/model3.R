...
mod_prior <- stan(file=file.path("model3_prior.stan"), 
...
util$plot_hist_quantiles(samples = mod_samples_prior,
                         bin_min = 0.5,
                         bin_max=3.5,
                         bin_delta = 1,
...
mod <- stan(file=file.path("model3.stan"), 
...
names <- c(
  ...
  paste0('beta[', 2:(nrow(player_first_games)),']'),
  'mu_beta',
  'tau_beta',
...
util$plot_disc_pushforward_quantiles(mod_samples,
                                     names,#names[sample(1:mod_data$N_players,100)],
                                     xlab="Player",
                                     ylab="Relative Skill")
...
util$plot_disc_pushforward_quantiles(mod_samples,
                                     names[sample(1:mod_data$N_players,100)],#names[sample(1:mod_data$N_players,100)],
                                     xlab="Player",
                                     ylab="Relative Aggression")


# Player aggression population parameters

# population location
par(mfrow=c(2,1), mar=c(5,5,1,1))
x <- seq(-0.1,1.7,by=0.1)
util$plot_expectand_pushforward(mod_samples[['mu_beta']],
                                50, flim=c(min(x),max(x)),
                                display_name="mu_beta")
lines(x, dnorm(x, 1, 1/2.32),col=c_mid_teal)

# population scale
x <- seq(0,1,by=0.1)
util$plot_expectand_pushforward(mod_samples[['tau_beta']],
                                50, flim=c(min(x),max(x)),
                                display_name="tau_beta")
lines(x, dnorm(x, 0, 0.5/2.32),col=c_mid_teal)



# A few players
par(mfrow=c(4, 2), mar=c(5, 5, 1, 1))
for (c in c(2, 53, 1111, 1232)) {
...
util$plot_conditional_mean_quantiles(mod_samples, pred_names,obs_xs = games$player_white_total_pre_game_tournament_score - games$player_black_total_pre_game_tournament_score, baseline_values = games$outcome_ordinal,display_ylim = c(0,3.5))
