...
model {
  ...
  for (i in 1:N_players) {
    d_matchup_ncp[i] ~ normal(0, 1);
  }
  
  mu_d ~ normal(0, 0.5);        // Prior centered at 0
  tau_d ~ normal(0, 0.5);       // Half-normal for global scale
  tau_player ~ normal(0, 0.5);  // Half-normal for player scales
  
  // LKJ prior for correlation matrix
  L_player ~ lkj_corr_cholesky(3.0);
