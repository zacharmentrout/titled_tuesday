...
parameters {
  vector[N_players - 1] gamma_free; // player skill 

  ordered[K - 1] cut_points; // Interior cut points
  
  // aggression parameters
  vector<lower=0>[N_players - 1] beta_tilde; 
  
  real<lower=0> mu_beta; // population location
  real<lower=0> tau_beta; // population scale
}

transformed parameters {
  vector[N_players] gamma = append_row([0]', gamma_free);
  
  vector[N_players] beta = append_row([1]', beta_tilde * tau_beta + mu_beta);
...
model {
  ...
  mu_beta ~ normal(1, 1/2.32);
  tau_beta ~ normal(0, 0.5/2.32);

  // Prior model
  cut_points ~ induced_dirichlet(rho/tau + rep_vector(1, K));

  // Observational model
  y ~ ordered_logistic( (beta[white_idxs] .* beta[black_idxs]).*(gamma[white_idxs] - gamma[black_idxs]), cut_points);
}
