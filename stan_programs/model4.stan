...
parameters {
  vector[N_players - 1] gamma_tilde; // player skill
  ordered[K - 1] cut_points; // Interior cut points
  
  real mu_gamma; // skill population location
  real<lower=0> tau_gamma; // skill population scale
  
}

transformed parameters {
  vector[N_players] gamma = append_row([0]', gamma_tilde * tau_gamma + mu_gamma);
...
model {
  ...
  mu_gamma ~ normal(0, 3/2.32);
  tau_gamma ~ normal(0, 3/2.57);
